...
func Build(paths infra_shared.Paths, mf *proto.Manifest, rpc shared.RPCInfo, repo string) string {
	fs := token.NewFileSet()
				...
				&ast.CompositeLit{
					Elts: []ast.Expr{
						...
						&ast.KeyValueExpr{
							...
							Value: &ast.Ident{Name: fmt.Sprintf("test%sHappy", rpc.Name)},
						},
					},
				},
			},
	...
	f := &ast.File{
		...
		Decls: []ast.Decl{
			...
			&ast.FuncDecl{
				Name: &ast.Ident{Name: fmt.Sprintf("Test%s", rpc.Name)},
				Type: &ast.FuncType{
			...
			&ast.FuncDecl{
				Name: &ast.Ident{Name: fmt.Sprintf("test%sHappy", rpc.Name)},
				Type: &ast.FuncType{
					Params: &ast.FieldList{
						List: []*ast.Field{
							&ast.Field{
